...
The ability to update a running production application without interrupting users is one of Kubernetes' most valuable features. No maintenance windows. No "we'll deploy at 3 AM." No scrambling to restart services manually. Just `kubectl apply` and Kubernetes handles the rest.

This lesson covers the full deployment lifecycle: strategy configuration, triggering updates, monitoring progress, pausing mid-rollout, and rolling back instantly when something goes wrong.

---

## How Rolling Updates Work: A Visual Walkthrough

Given a Deployment with `replicas: 3`, `maxSurge: 1`, `maxUnavailable: 0`:
...
At every step, the Service only routes to **Ready** pods. Users never experience a request to an unready or terminating pod.

---

## Step 1: Configure the Strategy

Every production Deployment should explicitly set its update strategy:
...
Edit `deployment.yaml`, change the image tag, then apply:
...
Sometimes you need to restart all pods (pick up new ConfigMap env vars, clear in-memory state):
...
Pause a rollout to manually validate the new version with partial traffic before completing:
...
For applications that cannot run two versions simultaneously (e.g., incompatible database schema changes):
...
A complete GitHub Actions workflow using `kubectl`:
...
- [ ] **Image tag is immutable** — use a specific SHA or semver tag, never `latest`
- [ ] **Readiness probe is configured** — without it, rolling updates are unsafe
- [ ] **`terminationGracePeriodSeconds`** is set to allow in-flight requests to complete
- [ ] **`minReadySeconds`** is set to catch pods that fail shortly after becoming Ready
- [ ] **`maxUnavailable: 0`** is set to guarantee zero downtime
- [ ] **`revisionHistoryLimit`** is set (default 10) — keep rollback history
- [ ] **`kubernetes.io/change-cause`** is annotated — maintain a human-readable audit trail
- [ ] **Database migrations are backward-compatible** — old app must work with new schema
- [ ] **Rollback procedure is documented** — team knows the command to run on failure
- [ ] **Monitoring dashboards are open** — watch error rates during and after rollout
...
Zero-downtime deployments rely on four things working together:
1. **`maxUnavailable: 0`** — never remove an old pod before a new one is Ready
2. **Readiness probe** — new pods only receive traffic once they signal Ready
3. **`terminationGracePeriodSeconds`** — old pods finish in-flight requests before dying
4. **`minReadySeconds`** — pods must be stable for N seconds before being counted as available

**Congratulations — you have completed the Kubernetes Basics course!**

You now have a deep understanding of every fundamental Kubernetes concept: the architecture, the CLI, Pods, Deployments, Services, ConfigMaps, Secrets, Ingress, Resource Management, Health Probes, Storage, and Rolling Updates. You are ready to build and operate production Kubernetes workloads — and for the next course, running lightweight production clusters with **K3s**.
