...
<KnowledgeCheck
  question="What allows 100+ Docker containers to run simultaneously on a single modest Linux server?"
  hint="Consider what containers share vs what VMs must duplicate."
  options={[
    { id: "1", text: "Each container runs its own lightweight hypervisor.", isCorrect: false },
    { id: "2", text: "All containers share the single underlying host OS kernel, eliminating duplicated Guest OS memory overhead.", isCorrect: true },
    { id: "3", text: "Docker compresses RAM using hardware swap exclusively.", isCorrect: false },
    { id: "4", text: "Containers only execute during active network requests.", isCorrect: false }
  ]}
  explanation="Containers share the host Linux kernel and isolate processes using Namespaces and cgroups. Because each container doesn't duplicate a 1-2GB Guest OS like VMs do, memory footprint is minimal and density is 10x higher."
/>
