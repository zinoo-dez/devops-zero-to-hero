...
Running Docker Compose in production requires a fundamentally different mindset than development. The priorities shift:
...
Never maintain separate copies of your Compose file. Use one **base file** with environment-specific **override files**:
...
When you run `docker compose -f docker-compose.yml -f docker-compose.prod.yml up`, the files are **deep merged** — the override file's values win on conflicts, and non-conflicting keys from both files are preserved.
...
Add free TLS certificates to your production stack:
...
Docker Compose doesn't have built-in zero-downtime rolling updates. These strategies minimize the gap:
...
Docker Compose on a single server is production-viable for many workloads. Here's how to decide:
...
Deploying Docker Compose to production requires:
- **Override files** (`docker-compose.prod.yml`) for environment-specific config — never duplicate the base file
- **Pre-built images** from a container registry — never `build:` on the production server
- **Secrets injected by CI/CD** — never committed to Git
- **Log rotation** with `json-file` driver — prevents disk exhaustion
- **Resource limits** — CPU and memory caps prevent the noisy-neighbor problem
- **HTTPS with Certbot** — free, automatic TLS certificates
- **Full GitHub Actions pipeline** — build, push to GHCR, deploy to server, verify health check
- **PostgreSQL backup** as a cron job — your data is not protected by Docker alone

Congratulations — you have completed the Docker Compose course. You are now ready for the Kubernetes Basics course, where you will learn how to orchestrate containers across multiple servers with automatic self-healing and zero-downtime deployments.
